...
export const removeSubscription= asyncHandler(async (req,res,next)=>{
       ...
       if(refundPeriod<=timeSinceFirstPayment){
       	  return next(new AppError('Period for refund is over , so no refund will be provided! ',400));
       }
...
export const allPayments=asyncHandler(async (req,res,next)=>{
    ...
    const allPayments= await razorpay.subscriptions.all(process.env.RAZORPAY_PLAN_ID,{
